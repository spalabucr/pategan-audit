...
from utils import fix_dtypes, featurize_df_naive, get_histogram_domain, featurize_df_histogram, featurize_df_corr, wb_get_auc_est_eps, bb_get_auc_est_eps
...
for pg_name in tqdm(pgs, desc="pg", leave=False):
    ...
    for i in tqdm(range(n_all), desc="pg_it", leave=False):
        ...
        synth_df_out = pg_model_out.generate(len(df_out))
        synth_df_out = fix_dtypes(synth_df_out, dtypes)
        synth_df_in = pg_model_in.generate(len(df_in))
    ...
    wb_t_auc, emp_eps_approxdp_t = wb_get_auc_est_eps(wb_t_out_probs, wb_t_in_probs, n_valid, n_test, delta)
    wb_auc, emp_eps_approxdp = wb_get_auc_est_eps(wb_out_probs, wb_in_probs, n_valid, n_test, delta)
    ...
    bb_na_auc, emp_eps_approxdp_na = bb_get_auc_est_eps(bb_na_out_data, bb_na_in_data, n_train, n_valid, n_test, delta)
    bb_hi_auc, emp_eps_approxdp_hi = bb_get_auc_est_eps(bb_hi_out_data, bb_hi_in_data, n_train, n_valid, n_test, delta)
    bb_co_auc, emp_eps_approxdp_co = bb_get_auc_est_eps(bb_co_out_data, bb_co_in_data, n_train, n_valid, n_test, delta)
    bb_en_auc, emp_eps_approxdp_en = bb_get_auc_est_eps(bb_en_out_data, bb_en_in_data, n_train, n_valid, n_test, delta)
